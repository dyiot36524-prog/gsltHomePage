...
시옷은 단순한 IoT 제어기가 아니다. **실시간 공간 예약과 예약 스케줄에 연동된 자동화**를
한 시스템으로 묶은 것이 핵심이다. 테니스코트·골프타석·볼링장·당구장·회의실처럼 시간 단위로
배정되는 공간에서, 예약이 확정되면 입장 QR이 발송되고, 현장에서 인증하면 그 공간의
조명·공조·스마트글라스가 스케줄에 맞춰 켜지며, 시간이 끝나면 꺼진다. 오지 않으면 노쇼로
자동 기록된다. 예약 시스템과 제어 시스템이 따로 놀 때 사람이 메우던 일을 없앤다.

평면도 위에 장비와 자동화를 배치해 관제하는 '도면 제어'도 함께 제공한다. 같은 회사의
비즈모아가 도면 위에서 견적을 내므로, 견적 단계의 도면이 그대로 운영 화면이 된다.
