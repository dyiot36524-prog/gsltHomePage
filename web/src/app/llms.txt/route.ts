...
시옷(SIOT)은 회사가 'Space Operating Platform'이라 부르는 **지능형 공간 운영 플랫폼**이다.
통합 관제·공간 예약·출입 연동·자동화·에너지 다섯 가지를 한 시스템에 올린다.

- 통합 관제: 전력 사용량과 추이, 온라인/오프라인 디바이스 수, 실내 CO₂ 등 공기질 센서,
  자동화 실행 현황이 한 대시보드에 실시간으로 모인다. 어디를 먼저 봐야 하는지 화면이 알려준다.
- 공간 예약 → 출입 → 제어: 관리자가 공간·시간·인원을 등록하면 층·회의실·타석별 타임라인에
  놓이고, 예약자에게 **그 시간에만 유효한 모바일 QR 출입권**이 나간다. 현장 단말에 인증하면
  문이 열리는 동시에 조명·공조가 켜지고 예약 프로그램이 실행된다.
- 도면 제어: 실제 평면도 위에 장비를 배치해 위치로 관제한다. 태블릿·PC·대형 터치DID·모바일에서
  권한을 분리해 실시간으로 연동된다. 같은 회사의 비즈모아가 도면 위에서 견적을 내므로,
  견적 단계의 도면이 그대로 운영 화면이 된다.
- 복합 레이어 자동화: 시각·재실·구역 조건이 겹치는 규칙을 건다. 예를 들어 "오전 8시에 모든 층을
  23℃로, 그 전에 움직임이 감지되면 그 구역만 먼저 23℃로, 밤 10시에 공용부를 제외하고 전등과 EHP를
  끄되 재실자가 있으면 끄지 말 것, 이후 움직임이 5분간 없으면 전원을 모두 차단" 같은 조합이 가능하다.
- 붙는 장비: 에어컨, 디밍조명, 매직글라스, 전원 플러그, 환경 센서, 재실·동작 센서, 적외선 리모컨,
  산업장비. Wi-Fi·블루투스·ZigBee·Z-Wave를 함께 지원해 제조사가 달라도 한 화면으로 묶는다.

적용 대상은 시간 단위로 배정되는 공간 전반이다 — 회의실, 테니스코트, 골프 타석, 볼링장, 당구장,
