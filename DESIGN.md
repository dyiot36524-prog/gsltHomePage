...
각 브랜드색은 **면으로 쓰는 값**과 **글자로 쓰는 값**이 따로 있다. gslt가 `gslt-500`(면) /
`gslt-700`(글자)로 나뉘는 것과 같은 구조다. 이 구분이 없으면 브랜드색이 그대로 작은 글자에
쓰여 3:1도 못 넘는다 — 감사에서 실제로 제품 면 전체가 이 방식으로 무너져 있었다.

| 제품 | 면 (배경) | 면 hover | 글자 (흰 바탕) |
|---|---|---|---|
| 시옷 | `siot-500` #f97316 | `siot-400` | `siot-700` #c2410c (5.18:1) |
| 비즈모아 | `bizmoa-500` #3b82f6 | `bizmoa-400` | `bizmoa-600` #2563eb (5.17:1) |
| 모락 | `morak-500` #00c2c2 | `morak-400` | `morak-800` #007070 (5.91:1) |

`morak-800`은 이 규칙을 위해 새로 만든 값이다. 기존 최대치 `morak-700`(#008585)이 흰 바탕에서
4.47:1로 본문 등급에 0.03 모자라, 모락만 작은 글자에 쓸 수 있는 브랜드색이 없었다.
...
**브랜드 면 위의 글자는 먹색이다 (The Ink-On-Brand Rule).** 위 규칙은 청록만의 예외가 아니라
네 브랜드 전부에 적용된다. 밝은 브랜드 면 + 먹색 글자 + **hover에 배경이 더 밝아지는** 방향이
이 시스템의 주 버튼 문법이다. 흰 글자는 어느 브랜드색 위에서도 본문 등급에 닿지 않는다 —
`morak-500` 2.21:1, `gslt-500` 2.40:1, `siot-500` 2.80:1, `bizmoa-500` 3.68:1.

| 제품 | 주 버튼 | 기본 → hover |
|---|---|---|
| gslt | `bg-gslt-500 hover:bg-gslt-400 text-slate-900` | 7.44:1 → 8.54:1 |
| 시옷 | `bg-siot-500 hover:bg-siot-400 text-slate-900` | 6.37:1 → 7.89:1 |
| 비즈모아 | `bg-bizmoa-500 hover:bg-bizmoa-400 text-slate-900` | 4.85:1 → 7.02:1 |
| 모락 | `bg-morak-500 hover:bg-morak-400 text-slate-900` | 8.07:1 → 9.75:1 |

**색은 계산해서 정한다 (The Measured Color Rule).** 이 문서의 비율은 장식이 아니라 검사 대상이다.
`web/scripts/contrast.mjs`가 토큰 이름과 알파를 받아 실제 비율을 낸다. 새 색을 쓰기 전에 돌린다.

```
node scripts/contrast.mjs slate-500 white          # 4.76:1
node scripts/contrast.mjs --alpha white 0.5 hero-bg # 반투명 글자의 실효 대비
node scripts/contrast.mjs --table                   # 전 토큰 × 주요 바탕
```
...
- **Active:** `gslt-500` 2px 밑줄 + 먹색 볼드 글자, 건수는 `gslt-700`.
  (건수는 0.75rem 작은 글자다. `gslt-600`은 3.66:1이라 아래 Don't 항목과 충돌해 `gslt-700`으로 맞췄다.)
...
- **Don't** 어떤 브랜드 면 위에도 흰 글자를 쓰지 않는다. 면은 밝게, 글자는 먹색, hover는 더 밝게다.
- **Don't** 제품 식별색의 `-500`(면 값)을 작은 글자에 쓰지 않는다. 글자는 `siot-700`·`bizmoa-600`·`morak-800`이다.
- **Don't** 검정·영상 위 반투명 흰 글자를 `white/50` 미만으로 두지 않는다. `/30`(2.54:1)·`/35`·`/40`·`/45`는
  배경이 무엇이든 본문 등급에 도달할 수 없다 — 알파가 대비의 상한을 정하기 때문이다.
- **Don't** 장식 글자(워터마크, 구분점, 불릿 글리프)를 대비만 올려 해결하지 않는다. 뜻을 옆 글자가
  지고 있으면 `aria-hidden="true"`로 장식임을 선언한다. 선언하지 않은 흐린 글자는 그냥 안 읽히는 글자다.
...
이 문서의 규칙은 주장이 아니라 검사 대상이다. `web/`에서 돌린다. 전부 종료 코드로 성패를 낸다.

| 스크립트 | 무엇을 증명하는가 |
|---|---|
| `node scripts/contrast.mjs --table` | 토큰 × 바탕 전수 대비표. 색을 정하기 전에 본다 |
| `node scripts/a11y-check.mjs` | 13개 라우트 × 3개 폭에서 **렌더된 화면의** 글자 대비, 접근 이름, 타겟 크기(WCAG 2.2 2.5.8), 제목 위계, 가로 넘침, **내부 링크 전수 200 확인** |
| `node scripts/kbd-check.mjs` | 마우스 없이 헤더 드롭다운 열기·이동·닫기·링크 실행 |
| `node scripts/form-check.mjs` | 문의 폼의 전송 중·접수 완료·입력 오류·과다 요청·네트워크 끊김 5가지 상태. 응답을 가로채 확인하므로 **운영 수신함에 테스트 문의가 남지 않는다** |
| `node scripts/perf-check.mjs` | 히어로 영상이 초기 로드 임계 경로에서 빠졌는지(바이트 실측), 헤더가 768~1100px에서 줄바꿈 없이 들어가는지 |
| `node scripts/scrub-check.mjs` | 영상 preload를 낮춘 뒤에도 스크롤 스크럽이 프레임을 실제로 바꾸는지 |

`a11y-check`는 소스 grep으로 알 수 없는 것을 본다 — Tailwind 클래스만 봐서는 조상이 합성한 실제
배경색을 알 수 없고, 배경이 그라데이션·영상이면 확정할 수도 없다. 그래서 판정을 셋으로 나눈다:
**실패**(대비 미달), **장식 선언**(`aria-hidden`이 붙어 부수적 텍스트로 처리된 것), **확인 불가**
(영상·그라데이션 위라 자동 판정이 불가능 — 사람이 봐야 하는 목록). 확인 불가를 통과로 세지 않는다.

**죽은 링크 검사가 특히 중요하다.** 이관 과정에서 사이트 전체 전환 CTA 10개가 `/?open=contact`를
가리킨 채 아무 데도 닿지 않은 적이 있다. 404가 아니라 홈으로 튕겨서 아무도 눈치채지 못했다.
`a11y-check`의 마지막 단계가 페이지에서 수집한 모든 내부 링크에 실제로 요청을 보내고, `#` 앵커는
대상 문서에 그 `id`가 있는지까지 확인한다. 링크는 눈으로 보지 말고 이 검사로 본다.

### 실패 안내의 원칙

문의 폼의 실패 안내는 **사용자가 스스로 할 수 있는 일이 있는가**로 갈린다.

- **입력 오류(400)** — 사용자가 고칠 수 있다. 무엇이 틀렸는지만 말한다.
  여기에 "전화로 연락해 주세요"를 붙이면 잘못된 안내다. 할 일은 이메일 형식을 고치는 것이지
  전화를 거는 게 아니다.
- **서버·네트워크 실패(429·5xx·연결 끊김)** — 사용자가 할 수 있는 게 없다. 이때만 전화·이메일이라는
  **다른 길**을 준다. 단, 서버 메시지에 이미 번호가 들어 있으면 또 붙이지 않는다.

숨길 때는 `hidden` 클래스가 아니라 렌더 자체를 하지 않는다. `display:none`은 화면에서만 지우고
`aria-live`가 읽어 가는 문자열에는 그대로 남아, 보이지 않는 안내가 소리로만 들리게 된다.
